...
export class PedidoShowComponent implements OnInit {
  ...
  downloadEtiquetas() {
    ...
    let docAdd = this.pedidoShow.idpedido;
    let nombreCliente = this.pedidoShow.nomcliente;
    let dirCliente = this.pedidoShow.clientedir;
    let numeroBultos = this.pedidoShow.nrobultos;
    let pedidoNrofactura = "";
  
    if (this.pedidoShow.nrofactura != undefined) {
      pedidoNrofactura = this.pedidoShow.nrofactura;
    } else {
    ...
    for (let i = 1; i<=numeroBultos;i++) {
      ...
      ticketfileId = `Pedido N° ${docAdd}, etiqueta ${i} de ${numeroBultos}`;
      //ticketfileId = parseInt(docAdd);
       ...
       /* if (ticketfileId != this.pedidoService.pedido_.idpedido && this.pedidoService.pedido_.idpedido) {
        ticketfileId = this.pedidoService.pedido_.idpedido;
       } */
 
       //console.log('fileId ',fileId);
       //const id = 'Order-'+ Math.random().toString(36).substring(2)+Date.now()+'.pdf';
 
       const idfile = ticketfileId +'.pdf';
       this.pedidoService.pedido_.pdfname = idfile;
       this.pedidoService.pedido_.pdfb64 = file;
     ...
     pdfMake.createPdf(ticketDefinition).download(`Pedido N° ${docAdd}, etiqueta ${i} de ${numeroBultos}.pdf`);
    }
  }
