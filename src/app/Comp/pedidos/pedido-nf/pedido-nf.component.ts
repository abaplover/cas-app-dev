...
@Component({
  selector: 'app-pedido-nf',
  templateUrl: './pedido-nf.component.html',
  styleUrls: ['./pedido-nf.component.css']
})
export class PedidoNfComponent implements OnInit {
...
onCancelar(pf?: NgForm,de?: number){
  if (de == 0){
    if (this.pedido_.ffactura !== undefined || this.pedido_.tipodoc !== undefined || this.pedido_.nrofactura !== undefined){
...
//let docAdd = 'PED' + (Math.floor(100000 + Math.random() * 900000)).toString() + Date.now().toString();
  ...
  onSubmitnf(pf?: NgForm){
    if(this.pedido_.uid != null){
      ...
      let ahora = new Date();
      this.pedido_.ffactura = new Date(this.pedido_.ffactura);
